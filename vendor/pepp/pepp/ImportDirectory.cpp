...
void ImportDirectory<bitsize>::addModuleImport(std::string_view module, std::string_view import, std::uint32_t* rva) {
    ...
    vsize = m_image->getPEHdr()
                .getOptionalHdr()
                .getDataDir(DIRECTORY_ENTRY_IMPORT)
                .Size;
    ...
    if (newSec = m_image->getSectionHdr(".pepp"); newSec.getName() == ".dummy") {
        ...
        m_image->appendSection(
            ".pepp",
            20 * PAGE_SIZE,
            SCN_MEM_READ | SCN_MEM_WRITE | SCN_CNT_INITIALIZED_DATA | SCN_MEM_EXECUTE,
            &newSec);
    ...
    m_image->getPEHdr()
        .getOptionalHdr()
        .getDataDir(DIRECTORY_ENTRY_IMPORT)
        .VirtualAddress = newSec.getVirtualAddress() + (10 * PAGE_SIZE);
    m_image->getPEHdr()
        .getOptionalHdr()
        .getDataDir(DIRECTORY_ENTRY_IMPORT)
        .Size = vsize + sizeof detail::Image_t<>::ImportDescriptor_t;
...
void ImportDirectory<bitsize>::addModuleImports(std::string_view module, std::initializer_list<std::string_view> imports, std::uint32_t* rva) {
    ...
    m_image->getPEHdr()
        .getOptionalHdr()
        .getDataDir(DIRECTORY_ENTRY_IMPORT)
        .VirtualAddress = newSec.getVirtualAddress() + PAGE_SIZE;
    m_image->getPEHdr()
        .getOptionalHdr()
        .getDataDir(DIRECTORY_ENTRY_IMPORT)
        .Size = vsize + sizeof detail::Image_t<>::ImportDescriptor_t;
